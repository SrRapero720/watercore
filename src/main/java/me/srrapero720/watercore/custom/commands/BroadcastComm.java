...
public class BroadcastComm {
    public static void register(@NotNull CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("broadcast-raw")
                ...
                .then(Commands.argument(new TranslatableComponent("wc.command.broadcast-raw.hint").getString(), MessageArgument.message())
                .executes(BroadcastComm::broadcastToServer))
        );

        dispatcher.register(Commands.literal("broadcast")
                .requires((p_137800_) -> p_137800_.hasPermission(3))
                .then(Commands.argument(new TranslatableComponent("wc.command.broadcast.hint").getString(), MessageArgument.message())
